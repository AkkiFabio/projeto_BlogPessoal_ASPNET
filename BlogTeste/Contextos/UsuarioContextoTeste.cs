...
namespace BlogTeste.Contextos
{
    ...
    public class UsuarioContextoTeste
    {
        ...
        public void InserirNovoUsuarioRetornaUsuarioInserido()
        {
            ...
            var resultado = _contexto.Usuarios.FirstOrDefault(u => u.Email == "gustavo@email.com");

            //Código para procurar resultado que possui as letras apontadas
            //var resultado1 = _contexto.Usuarios.Where(u => u.Nome.Contains("stav")).ToList();

            // ENTÃO - Então deve retornar resultado nao nulo
            Assert.IsNotNull(resultado);
            Assert.AreEqual("Gustavo Boaz", resultado.Nome);
        }
        //Teste unitário Read (cRud):
        [TestMethod]
